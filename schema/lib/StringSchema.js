...
class StringSchema extends BaseSchema_1.default {
    constructor(config = {}) {
        ...
        if ((0, BaseSchema_1.isDefined)(this.format))
            assert(['email', 'url', 'date', 'time'].includes(this.format), 'format must be one of email, url, date, time');
        ...
        if ((0, BaseSchema_1.isDefined)(this.enum)) {
            ...
            assert(this.enum.every(x => typeof x === 'string'), 'enum must contain only strings');
        }
